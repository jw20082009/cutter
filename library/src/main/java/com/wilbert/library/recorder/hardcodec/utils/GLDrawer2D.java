...
package com.wilbert.library.recorder.hardcodec.utils;
...
public class GLDrawer2D {
    ...
    private static final String fss_2 = "precision mediump float;\n"
    ...
    public void setRecordEffect(float[] rect) {
        if (rect == null)
            return;
        showRect[0] = rect[0];
        showRect[1] = rect[1];
     ...
     */
    public GLDrawer2D() {
        pVertex = ByteBuffer.allocateDirect(VERTEX_SZ * FLOAT_SZ).order(ByteOrder.nativeOrder())
                .asFloatBuffer();
        pVertex.put(VERTICES);
        pVertex.flip();
        pTexCoord = ByteBuffer.allocateDirect(VERTEX_SZ * FLOAT_SZ).order(ByteOrder.nativeOrder())
                .asFloatBuffer();
        pTexCoord.put(TEXCOORD);
        pTexCoord.flip();

        hProgram = loadShader(vss, fss_2);
        GLES20.glUseProgram(hProgram);
        maPositionLoc = GLES20.glGetAttribLocation(hProgram, "aPosition");
        maTextureCoordLoc = GLES20.glGetAttribLocation(hProgram, "aTextureCoord");
        muMVPMatrixLoc = GLES20.glGetUniformLocation(hProgram, "uMVPMatrix");
        muTexMatrixLoc = GLES20.glGetUniformLocation(hProgram, "uTexMatrix");
        mShowRect = GLES20.glGetUniformLocation(hProgram, "showRect");
     ...
     * @param tex_matrix texture matrix、if this is null, the last one use(we don't
     *                   check size of this array and needs at least 16 of float)
     */
    public void draw(final int tex_id, final float[] tex_matrix) {
        ...
        GLES20.glUniformMatrix4fv(muMVPMatrixLoc, 1, false, mMvpMatrix, 0);
        GLES20.glUniform4f(mShowRect, showRect[0], showRect[1], showRect[2], showRect[3]);

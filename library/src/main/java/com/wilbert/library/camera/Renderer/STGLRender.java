...
import com.wilbert.library.frameprocessor.glutils.GlUtil;
...
public class STGLRender {
    ...
    private final static String SHOWRECT_UNIFORM = "showRect";

    private float[] showRect = new float[]{
            0f, 0f, 1.0f, 1.0f
    };
    ...
    private ArrayList<HashMap<String, Integer>> mArrayPrograms = new ArrayList<HashMap<String, Integer>>(
            2) {
        {
            for (int i = 0; i < 2; ++i) {
    ...
    public STGLRender() {
        ...
        mGLSaveTextureBuffer = ByteBuffer
                .allocateDirect(TextureRotationUtil.TEXTURE_NO_ROTATION.length * 4)
                .order(ByteOrder.nativeOrder()).asFloatBuffer();
    ...
    private void initProgram(String fragment, HashMap<String, Integer> programInfo) {
        ...
        if (proID == 0) {
            ...
            programInfo.put(POSITION_COORDINATE,
                    GLES20.glGetAttribLocation(proID, POSITION_COORDINATE));
            programInfo.put(TEXTURE_UNIFORM, GLES20.glGetUniformLocation(proID, TEXTURE_UNIFORM));
            programInfo.put(TEXTURE_COORDINATE,
                    GLES20.glGetAttribLocation(proID, TEXTURE_COORDINATE));
        }
    }
    ...
    private void initYUVProgram(String vertext, String fragment) {
        ...
        showRectLoc = GLES20.glGetUniformLocation(YUVToRGBAProgramId, SHOWRECT_UNIFORM);
    }

    public void initDrawPoints() {
        mDrawPointsProgram = OpenGLUtils.loadProgram(DRAW_POINTS_VERTEX_SHADER,
                DRAW_POINTS_FRAGMENT_SHADER);
    ...
    public void adjustVideoTextureBuffer(int orientation, boolean flipHorizontal,
                                         boolean flipVertical) {
        float[] textureCords = TextureRotationUtil.getVideoRotation(orientation, flipHorizontal,
                flipVertical);
        if (mTextureBuffer == null) {
            mTextureBuffer = ByteBuffer.allocateDirect(textureCords.length * 4)
                    .order(ByteOrder.nativeOrder()).asFloatBuffer();
    ...
    /**
     * 此函数有三个功能 1. 将OES的纹理转换为标准的GL_TEXTURE_2D格式 2.
     * 将纹理宽高对换，即将wxh的纹理转换为了hxw的纹理，并且如果是前置摄像头，则需要有水平的翻转 3.
     * 读取上面两个步骤后的纹理内容存储到RGBA格式的buffer中，若初始化时使用resize参数，则读取resize后的纹理内容
     *
     ...
     * @param textureId [in] OES的纹理id
     * @param buffer    [out] RGBA的buffer
     * @param bufIndex  [in] 使用的buffer索引，使用CameraDisplaySingleInputMultiThread时有效
     * @return 转换后的GL_TEXTURE_2D的纹理id
     */
    public int preProcess(int textureId, ByteBuffer buffer, int bufIndex) {
        ...
        if (buffer != null) {
            if (mNeedResize) {
                ...
                GLES20.glBindFramebuffer(GLES20.GL_FRAMEBUFFER, mFrameBuffersResize[bufIndex]);
                GLES20.glDrawArrays(GLES20.GL_TRIANGLE_STRIP, 0, 4);
                GLES20.glReadPixels(0, 0, mWidthResize, mHeightResize, GLES20.GL_RGBA,
                        GLES20.GL_UNSIGNED_BYTE, buffer);
                GLES20.glBindFramebuffer(GLES20.GL_FRAMEBUFFER, 0);
            } else {
                GLES20.glReadPixels(0, 0, mViewPortWidth, mViewPortHeight, GLES20.GL_RGBA,
                        GLES20.GL_UNSIGNED_BYTE, buffer);
            }
        }
    ...
    public int YUV2RGB(int textureIdY, int textureIdUV, boolean flag) {
        ...
        GLES20.glVertexAttribPointer(glAttribTextureCoordinate, 2, GLES20.GL_FLOAT, false, 0,
                mTextureBuffer);
        GLES20.glEnableVertexAttribArray(glAttribTextureCoordinate);
        GLES20.glUniform4f(showRectLoc, showRect[0],showRect[1], showRect[2], showRect[3]);
        if (textureIdY != -1) {
            GLES20.glActiveTexture(GLES20.GL_TEXTURE0);
    ...
    public void onDrawPoints(int textureId, float[] points) {
        ...
        buff = ByteBuffer.allocateDirect(points.length * 4).order(ByteOrder.nativeOrder())
                .asFloatBuffer();
    ...
    private void initFrameBuffers(int width, int height) {
        ...
        if (mSavePictureFrameBuffers == null) {
            ...
            bindFrameBuffer(mSavePictureFrameBufferTextures[0], mSavePictureFrameBuffers[0], width,
                    height);
        }
        ...
        if (mNeedResize && mFrameBuffersResize == null) {
            ...
            bindFrameBuffer(mFrameBufferTexturesResize[0], mFrameBuffersResize[0], mWidthResize,
                    mHeightResize);
            bindFrameBuffer(mFrameBufferTexturesResize[1], mFrameBuffersResize[1], mWidthResize,
                    mHeightResize);
        }
    }
    ...
    public void setShowRect(float left, float top, float right, float bottom) {
        showRect[0] = left;
        showRect[1] = top;
        showRect[2] = right;
        showRect[3] = bottom;

...
/**
 * 相机模式指示器 Created by cain.huang on 2017/12/28.
 */
...
public class HorizontalIndicatorView extends View {
    ...
    private List<String> mIndicators = new ArrayList<String>(Arrays.asList("拍照", "单击拍", "长按拍"));// 数据源字符串数组
    ...
    private void initAttrs(AttributeSet attrs) {
        TintTypedArray tta = TintTypedArray.obtainStyledAttributes(getContext(), attrs,
                R.styleable.sample_IndicatorView);
        // 两种字体颜色和字体大小
        mSeeSize = tta.getInteger(R.styleable.sample_IndicatorView_SeeSize, 5);
        mSelectedTextSize = tta.getFloat(R.styleable.sample_IndicatorView_SelectedTextSize, 50);
        mSelectedColor = tta.getColor(R.styleable.sample_IndicatorView_SelectedTextColor,
                mContext.getResources().getColor(android.R.color.black));
        mTextSize = tta.getFloat(R.styleable.sample_IndicatorView_TextSize, 40);
        mTextColor = tta.getColor(R.styleable.sample_IndicatorView_TextColor,
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mCurrentIndex >= 0 && mCurrentIndex <= mIndicators.size() - 1) {
            ...
            canvas.drawText(mIndicators.get(mCurrentIndex),
                    getWidth() / 2 - mCenterTextWidth / 2 + mOffset,
            ...
            for (int i = 0; i < mIndicators.size(); i++) {
                ...
                if (i == 0) {
                    mTextPaint.getTextBounds(mIndicators.get(0), 0, mIndicators.get(0).length(),
                            mRect);
                    mTextHeight = mRect.height();
                }
                if (i != mCurrentIndex) {
                    canvas.drawText(
                            mIndicators.get(i), (i - mCurrentIndex) * mFontSize + getWidth() / 2
                                    - mTextWidth / 2 + mOffset,

...
import com.wilbert.library.recorder.hardcodec.utils.EGLBase;
import com.wilbert.library.recorder.hardcodec.utils.GLDrawer2D;
...
public final class RenderHandler implements Runnable {
    ...
    public void setRecordEffect(float[] rect) {
        mDrawRect = rect;
        if (rect == null || mDrawer == null)
            return;
        mDrawer.setRecordEffect(rect);
    }
...
//					mSync.notifyAll();
            ...
            if (localRequestDraw) {
                if ((mEgl != null) && mTexId >= 0) {
                    ...
                    Log.i("PreviewTracer", "RenderHandler 1:" + mTexId);
                    mDrawer.draw(mTexId, mMatrix);
                    Log.i("PreviewTracer", "RenderHandler 2:" + mTexId);
                    mInputSurface.swap();
                }
            } else {
                synchronized (mSync) {
    ...
    private final void internalPrepare() {
        ...
        if(mDrawRect != null){
            mDrawer.setRecordEffect(mDrawRect);
        }
        mSurface = null;
        mSync.notifyAll();
    }

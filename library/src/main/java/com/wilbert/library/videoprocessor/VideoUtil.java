...
public class VideoUtil {
    ...
    public static void seekToFirstFrame(MediaExtractor extractor, int trackIndex, int durationMs) {
        int seekToDuration = 0;//durationMs * 1000;
        if (extractor.getSampleTrackIndex() != trackIndex) {
            extractor.selectTrack(trackIndex);
        }
        extractor.seekTo(seekToDuration, MediaExtractor.SEEK_TO_NEXT_SYNC);
        while (seekToDuration < durationMs * 1000 && extractor.getSampleTrackIndex() != trackIndex) {
            seekToDuration += 10000;
            extractor.seekTo(seekToDuration, MediaExtractor.SEEK_TO_NEXT_SYNC);
        }
    }
